...
export default function StatsBar() {
  return (
    <section className="relative bg-zinc-950/60 border-y border-white/5 py-12 md:py-16 overflow-hidden">
        ...
        <StaggerChildren className="grid grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          {statsData.map((stat, i) => (
            ...
            >
              ...
              <span className="text-xs text-zinc-500 mt-2 font-medium tracking-wider uppercase max-w-[180px]">
                {stat.label}
              </span>
            </div>
          ))}
      ...
      <FadeUp delay={0.3} className="w-full relative flex items-center justify-center py-4 bg-black/40 border-t border-white/5">
        ...
        <div className="flex w-[200%] animate-marquee overflow-hidden whitespace-nowrap">
          {/* First slide */}
          <div className="flex justify-around min-w-full items-center gap-12 text-zinc-600 text-xs md:text-sm font-semibold tracking-widest uppercase">
          ...
          <div className="flex justify-around min-w-full items-center gap-12 text-zinc-600 text-xs md:text-sm font-semibold tracking-widest uppercase">
            {clients.map((client, idx) => (
              <span key={idx + clients.length} className="hover:text-brand-cyan transition-colors duration-300">
                {client}
              </span>
            ))}
          </div>
